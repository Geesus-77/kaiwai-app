...
(() => {
  ...
  // ── 2단계 자동 인증 상태 ──
  let capPhoneDone = false;
  let capBankDone  = false;

  function resetCaptainVerify() {
    capPhoneDone = false;
    capBankDone  = false;
    // STEP 1 초기화
    const ps = $("capPhoneStatus");
    if (ps) { ps.textContent = "(미완료)"; ps.className = "cap-step__status cap-step__status--no"; }
    const pb = $("capPhoneBtn");
    if (pb) { pb.disabled = false; pb.textContent = "PASS / SMS 인증"; }
    $("capStep1") && $("capStep1").classList.remove("cap-step--done");
    // STEP 2 초기화
    const bs = $("capBankStatus");
    if (bs) { bs.textContent = "(미완료)"; bs.className = "cap-step__status cap-step__status--no"; }
    ["capBank","capAccount","capDepositName"].forEach(id => { const el = $(id); if (el) { el.value = ""; el.disabled = false; } });
    const sw = $("capSendWon"); if (sw) sw.disabled = false;
    const vw = $("capVerifyWrap"); if (vw) vw.style.display = "none";
    $("capStep2") && $("capStep2").classList.remove("cap-step--done");
    if ($("capAgree")) $("capAgree").checked = false;
    updateCapFinal();
  }

  // 최종 버튼: 2단계 인증 + 동의 모두 완료 시에만 활성화
  function updateCapFinal() {
    const agreed = $("capAgree") && $("capAgree").checked;
    const ok = capPhoneDone && capBankDone && agreed;
    const btn = $("capFinalBtn");
  ...
  // STEP 1 — 휴대폰 본인인증 (Mock)
  const capPhoneBtn = $("capPhoneBtn");
  if (capPhoneBtn) capPhoneBtn.addEventListener("click", () => {
    alert("본인인증이 완료되었습니다.");
    capPhoneDone = true;
    const ps = $("capPhoneStatus");
    ps.textContent = "(인증 완료 ✅)";
    ps.className = "cap-step__status cap-step__status--ok";
    capPhoneBtn.disabled = true;
    capPhoneBtn.textContent = "완료됨";
    $("capStep1").classList.add("cap-step--done");
    updateCapFinal();
  });

  // STEP 2 — 1원 보내기 → 입금자명 입력칸 노출
  const capSendWon = $("capSendWon");
  if (capSendWon) capSendWon.addEventListener("click", () => {
    if (!$("capBank").value) { toast("은행을 선택해 주세요"); return; }
    if (!$("capAccount").value.trim()) { toast("계좌번호를 입력해 주세요"); return; }
    $("capVerifyWrap").style.display = "block";
    $("capSendWon").disabled = true;
    $("capDepositName").focus();
  });

  // STEP 2 — 입금자명 인증하기 (Mock)
  const capVerifyBtn = $("capVerifyBtn");
  if (capVerifyBtn) capVerifyBtn.addEventListener("click", () => {
    if ($("capDepositName").value.trim().length < 3) { toast("입금자명 끝 3자리를 입력해 주세요"); return; }
    alert("계좌 실명 인증이 완료되었습니다.");
    capBankDone = true;
    const bs = $("capBankStatus");
    bs.textContent = "(인증 완료 ✅)";
    bs.className = "cap-step__status cap-step__status--ok";
    ["capBank","capAccount","capDepositName","capSendWon","capVerifyBtn"].forEach(id => { const el = $(id); if (el) el.disabled = true; });
    $("capStep2").classList.add("cap-step--done");
    updateCapFinal();
  });

  if ($("capAgree")) $("capAgree").addEventListener("change", updateCapFinal);

  if (capForm) capForm.addEventListener("submit", (e) => {
    e.preventDefault();
    if (!(capPhoneDone && capBankDone && $("capAgree").checked)) return;
    // 자동 승인 — 인증 결과를 저장해 공구 개설(index.html)에서 바로 사용
    try { localStorage.setItem("kaiwai_verified_host", "1"); } catch (_) {}
    closeCaptain();
    alert("축하합니다! 총대 권한이 자동 승인되었습니다.");
  });
